...
<?php if ( $matches ) : ?>
<div class="matchlist jquery-ui-tabs">
	<ul class="tablist">
	<?php for ($i = 1; $i <= $league->num_match_days; $i++) : ?>
		<li><a href="#match_day_tab_<?php echo $i ?>"><?php printf(__('%d. Match Day', 'leaguemanager'), $i) ?></a></li>
	<?php endfor; ?>
	</ul>
	
	<?php for ($i = 1; $i <= $league->num_match_days; $i++) : ?>
	<div id="match_day_tab_<?php echo $i ?>" class="jquery-ui-tab">
		<h3 class="tab-header"><?php printf(__('%d. Match Day', 'leaguemanager'), $i) ?></h3>
